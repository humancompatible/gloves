...
dnn_criteria = {
    ...
    'attrition': nn.CrossEntropyLoss(weight=torch.tensor([1, 8.0])),
}
...
class DNN(nn.Module):
    ...
    def predict_proba(self, x):
        ...
        # Forward pass to get raw output values
        raw_outputs = self.layers(x)

        # Apply softmax to convert raw outputs to probabilities
        probabilities = self.sm(raw_outputs)
